...
class Slot < ApplicationRecord
  ...
  def validate_duration_in_minutes
    unless [15, 30, 60].include? DURATION_IN_MINUTES
      raise ArgumentError, "DURATION_IN_MINUTES '#{DURATION_IN_MINUTES}' failed validation."
    end
  end

  def validate_times(*times)
    times.each do |time|
      unless regex_match_by_time_format(time)
